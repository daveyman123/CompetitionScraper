...
class MySpider(scrapy.Spider):
        name = "PublicStorageClarkson"

        
	
	allowed_domains = ['https://www.publicstorage.com']
	start_urls = ['https://www.publicstorage.com/missouri/self-storage-ellisville-mo/63011-self-storage/1036?lat=38.58888&lng=-90.60405&location=ellisville']

	def parse(self, response):
                display = Display(visible=0, size=(800, 600))
                display.start()

                url="https://www.publicstorage.com/missouri/self-storage-ellisville-mo/63011-self-storage/1036?lat=38.58888&lng=-90.60405&location=ellisville"
		...
		for n in range(len(sizeTagz)):
                ...
                #.replace('\n', '')
                        "special": "Broken",#re.sub(r"(?<=[a-z])\r?\n"," ",(specialTagz[n]).get_text()) + re.sub(r"(?<=[a-z])\r?\n"," ",(specialTagz2[n]).get_text()),
